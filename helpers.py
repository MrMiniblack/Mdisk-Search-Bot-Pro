...
async def main_convertor_handler(c: Client, message: Message, type: str, edit_caption: bool = False):
    ...
    if message.reply_markup:  # reply markup - button post
        ...
        if message.text:
            ...
            if message.photo:
                await message.reply_photo(photo=message.photo.file_id, caption=shortenedText, reply_markup=InlineKeyboardMarkup(buttsons))
            elif message.document:
                await message.reply_document(document=message.document.file_id, caption=shortenedText, reply_markup=InlineKeyboardMarkup(buttsons))
